...
<template>
  <div class="flex flex-col justify-center mt-4 gap-4">
    ...
    <div v-else class="flex flex-col grow container">
      <h1 class="font-Niramit text-3xl bold">All Products</h1>
    </div>
    <div class="flex flex-wrap gap-3 justify-center">
      <RouterLink
        :to="$route.path.includes('admin') ? `/admin/item/${game.id}` : `/products/${game.id}`"
        class="w-1/6 text-center bg-slate-700 overflow-hidden rounded-xl pb-2 hover:scale-105 duration-300"
        v-for="game in games"
        :key="game.id"
      >
        <div class="aspect-square flex items-center relative w-full overflow-hidden">
          <img class="top-0 h-full w-auto" :src="`${game.image}`" alt="" />
        </div>
        <h3 class="pt-3 text-xl">{{ game.name }}</h3>
      </RouterLink>
    </div>
  </div>
</template>
